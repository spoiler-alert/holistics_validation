...
class SQLValidator:
    ...
    def start_validation(self, model, overrides):
        ...
        if model["sql"] is None:
            ...
            if "{%" in model["sql"] or "{{" in model["sql"]:
                logger.warning(
                    "We don't currently support validating parameterized queries or queries that reference other models, skipping validation for %s",
                    model["name"],
                )
                return
    ...
    def create_field_dicts(self, model_name, dimensions, measures, table_name):
        dependent_fields = {}
        input_dict = {"dimensions": dimensions, "measures": measures}
        self.field_dicts = {"dimensions": {}, "measures": {}}
        ...
        for field_type in ["dimensions", "measures"]:
                ...
                if field_meta["syntax"] == "sql":
                    ...
                    if field_type == "measures" and field_aggregation_type != "custom":
                        if field_aggregation_type in aggregation_dict:
                            ...
                            )  ## TODO: explain why each type is not implemented, would depend on the sql engine
            ...
            while True:
                ...
                if len(dependent_fields) < dependent_field_len:
                    ...
                    logger.error(
                        "For %s %s: The SQL fields remaining are dependent on either other fields that aren't defined in SQL (they could be defined as AQL, which would break) OR are dimensions dependent on measures: %s",
                        model_name,
                        field_type,
                        dependent_fields,
                    )
                    raise ReferencesUndefinedSQL(
