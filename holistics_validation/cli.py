...
def sql_parser_add_args(parser):
    ...
    parser.add_argument(
        "--commit_oid",
        help="Takes precendence over branch name. Leaving both unspecified means it will test against production.",
    )
    parser.add_argument(
        "--branch_name",
        help="Used only if commit id is not specified. Leaving both unspecified means it will test against production.",
    )
    parser.add_argument(
...
def dashboard_parser_add_args(parser):
    ...
    parser.add_argument(
        "--dashboard_ids",
        help="A single dashboard ID or comma separated list of dashboard IDs to validate",
        required=True,
    )
...
def create_parser():
    ...
    reporting_parser = subparsers.add_parser(
        "reporting",
        help="Validates if a given commit oid or branch will break any pre-existing quick / legacy dashboards",
    )
